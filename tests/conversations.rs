...
#[test]
fn get_conversations_returns_conversations_from_user_id_header() {
    ...
    insert_test_message(&server, 1.to_string(), 2.to_string());
    insert_test_message(&server, 1.to_string(), 3.to_string());
    insert_test_message(&server, 1.to_string(), 2.to_string());
    ...
    let users = get_conversations.into_json::<Vec<UserID>>().unwrap();
